...
string PhongScene::getProgressInfo() {
    ...
    if (progress < 100) {
        ...
        char buffer[200];
        sprintf(buffer, "%d%% (pixels: %d / %d, light samples: %d / %d), speed: %d pixels / s, %s remaining",
            progress,
...
vec3 PhongMaterial::calculateLighting(Scene* scene, Ray& ray, vec3 position) {
    ...
    for (int i = 0; i < lights.size(); i++) {
        ...
        if (shadowRay.checkIfBlocked(scene->getObjects(), lightPosition))
            continue;
        local += calculatePhongShading(position, lightPosition, lights[i]->color);
    }
...
vec3 PhongMaterial::calculatePhongShading(vec3 position, vec3 lightPosition, vec3 lightColor) {
...
vec3 OpticalMaterial::calculateLighting(Scene* scene, Ray& ray, vec3 position) {
    ...
    vec3 Le(0);
    vec3 lightPosition = light->samplePosition();
...
vec3 OpticalMaterial::calculateBrdfShading(float F0, vec3 Le, vec3 n, float pdf, vec3 p, vec3 w_i, vec3 w_o) {
    ...
    float fd = (1 / PI)
        * (1 + (FD90 - 1) * pow((1 - (w_idotn)), 5) * (1 + (FD90 - 1) * pow((1 - (w_odotn)), 5) * (1 - metallic)));
...
float Triangle::intersects(Ray* ray) {
    ...
    if (dot(n_normalized, cross(b - a, p - a)) < 0 || dot(n_normalized, cross(c - b, p - b)) < 0 || dot(n_normalized, cross(a - c, p - c)) < 0)
        return -1;
